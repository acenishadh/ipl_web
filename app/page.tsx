import Link from 'next/link'
import { FlowAmbient } from '@/components/FlowAmbient'

export default function HomePage() {
  return (
    <main className="page-shell relative min-h-screen overflow-hidden">
      <FlowAmbient variant="home" />

      <div className="relative mx-auto flex max-w-5xl flex-col gap-12 px-1 py-10 sm:gap-16 sm:px-2 sm:py-14">
        <header className="page-enter flex flex-col items-start gap-4">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/35 bg-gradient-to-r from-cyan-500/15 to-violet-500/10 px-4 py-2 text-xs font-bold tracking-widest text-cyan-200 uppercase shadow-[0_0_20px_rgba(34,211,238,0.15)]">
            <span className="h-2 w-2 animate-pulse rounded-full bg-cyan-400 shadow-[0_0_8px_#22d3ee]" />
            IPL Arcade · Live rooms
          </div>
          <h1
            className="font-display text-balance text-4xl font-bold leading-[1.12] tracking-tight text-white sm:text-6xl md:text-7xl"
            style={{ textShadow: '0 0 48px rgba(34,211,238,0.15)' }}
          >
            Play IPL
            <br />
            <span
              className="bg-gradient-to-r from-cyan-300 via-fuchsia-300 to-amber-300 bg-clip-text text-transparent"
              style={{ textShadow: '0 0 40px rgba(34,211,238,0.25)' }}
            >
              with friends.
            </span>
          </h1>
          <p className="max-w-xl text-base leading-relaxed text-white/65 sm:text-lg">
            Run a live auction draft or a franchise cricket room in one place. Share a code, pick your team, and
            play while you trash-talk in the group chat — no sign-up, no install.
          </p>
        </header>

        <section className="stagger-children grid gap-4 sm:grid-cols-3 sm:gap-5">
          <div className="neon-card-cyan flex flex-col gap-5 overflow-hidden rounded-3xl border p-5 sm:p-6">
            <div className="text-4xl hero-float select-none" aria-hidden>
              🏏
            </div>
            <div>
              <h2 className="font-display text-xl font-bold text-white">Auction</h2>
              <p className="mt-1.5 text-sm leading-relaxed text-white/60">
                Host a timed Mega or Mock draft. 120 Cr purse — build a squad under pressure.
              </p>
            </div>
            <Link
              href="/create"
              className="tap-target mt-auto inline-flex min-h-[48px] items-center justify-center rounded-2xl px-5 py-3 text-sm font-bold text-slate-950 transition-transform active:scale-95 sm:min-h-0"
              style={{
                background: 'linear-gradient(135deg, #22d3ee 0%, #06b6d4 100%)',
                boxShadow: '0 0 24px rgba(34,211,238,0.4), 0 4px 16px rgba(0,0,0,0.3)',
              }}
            >
              Host auction →
            </Link>
          </div>

          <div className="neon-card-purple flex flex-col gap-5 overflow-hidden rounded-3xl border p-5 sm:p-6">
            <div className="text-4xl hero-float-delay select-none" aria-hidden>
              🎟️
            </div>
            <div>
              <h2 className="font-display text-xl font-bold text-white">Join draft</h2>
              <p className="mt-1.5 text-sm leading-relaxed text-white/60">
                Paste the code from your host, claim a franchise, and bid live.
              </p>
            </div>
            <Link
              href="/join"
              className="tap-target mt-auto inline-flex min-h-[48px] items-center justify-center rounded-2xl border-2 px-5 py-3 text-sm font-bold transition-transform active:scale-95 sm:min-h-0"
              style={{
                borderColor: 'rgba(192,132,252,0.55)',
                color: '#e9d5ff',
                background: 'linear-gradient(135deg, rgba(168,85,247,0.2) 0%, rgba(109,40,217,0.12) 100%)',
                boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.08)',
              }}
            >
              Join with code →
            </Link>
          </div>

          <div className="neon-card-pink flex flex-col gap-5 overflow-hidden rounded-3xl border p-5 sm:p-6">
            <div className="text-4xl hero-float select-none" aria-hidden>
              🏟️
            </div>
            <div>
              <h2 className="font-display text-xl font-bold text-white">Cricket mode</h2>
              <p className="mt-1.5 text-sm leading-relaxed text-white/60">
                Host sets the format: friendly duels or a full IPL-style season with points, caps, and playoffs.
              </p>
            </div>
            <Link
              href="/cricket"
              className="tap-target mt-auto inline-flex min-h-[48px] items-center justify-center rounded-2xl border-2 px-5 py-3 text-sm font-bold transition-transform active:scale-95 sm:min-h-0"
              style={{
                borderColor: 'rgba(244,114,182,0.55)',
                color: '#fbcfe8',
                background: 'linear-gradient(135deg, rgba(236,72,153,0.2) 0%, rgba(190,24,93,0.12) 100%)',
              }}
            >
              Enter cricket hub →
            </Link>
          </div>
        </section>

        <section className="stagger-children grid gap-3 sm:grid-cols-3 sm:gap-4">
          <div className="flex items-start gap-4 rounded-2xl border border-amber-400/25 bg-gradient-to-br from-amber-500/8 to-orange-500/5 p-5 transition-transform sm:hover:-translate-y-0.5">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-amber-400/15 text-xl shadow-inner">
              ⚡
            </div>
            <div>
              <div className="font-display text-sm font-bold text-amber-200">Low latency</div>
              <div className="mt-1 text-xs leading-relaxed text-white/55">WebSockets keep bids and ball picks in sync.</div>
            </div>
          </div>

          <div className="flex items-start gap-4 rounded-2xl border border-cyan-400/25 bg-gradient-to-br from-cyan-500/8 to-sky-500/5 p-5 transition-transform sm:hover:-translate-y-0.5">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-cyan-400/15 text-xl shadow-inner">
              💰
            </div>
            <div>
              <div className="font-display text-sm font-bold text-cyan-200">IPL purse rules</div>
              <div className="mt-1 text-xs leading-relaxed text-white/55">Auction purses and squad limits match the vibe.</div>
            </div>
          </div>

          <div className="flex items-start gap-4 rounded-2xl border border-emerald-400/25 bg-gradient-to-br from-emerald-500/8 to-teal-500/5 p-5 transition-transform sm:hover:-translate-y-0.5">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-400/15 text-xl shadow-inner">
              🏆
            </div>
            <div>
              <div className="font-display text-sm font-bold text-emerald-200">Bragging rights</div>
              <div className="mt-1 text-xs leading-relaxed text-white/55">
                Leaderboards and finals screens worth screenshotting when you win.
              </div>
            </div>
          </div>
        </section>

        <footer className="border-t border-white/[0.06] pt-8 text-center">
          <p className="site-footer-hint">IPL Arcade · Built for watch parties & league nights</p>
        </footer>
      </div>
    </main>
  )
}
